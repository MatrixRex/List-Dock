...
1.  **Dependencies**: Install `vite-plugin-pwa`.
2.  **Configuration**: 
    - Configure `vite.config.ts` to support both CRXJS (Extension) and PWA builds.
    - Generate PWA icons and `manifest.webmanifest`.
3.  **Platform Hook**: Create `usePlatform.ts` to detect `isExtension`, `isMobilePWA`, or `isDesktopWeb`.
4.  **Deployment**: Set up a CI/CD pipeline (GitHub Actions) to deploy the web version to GitHub Pages.
...
1.  **App Shell**: Create a top-level `LayoutSwitcher` component.
2.  **Layouts**: 
...
3.  **Navigation**: Implement a responsive navigation system that switches from a Sidebar (Desktop) to a Bottom Bar (Mobile).
